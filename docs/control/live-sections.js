(function () {
  ...
  async function saveShopItem(event){event.preventDefault();const id=Number(document.querySelector("#shopItemId").value),payload={title:document.querySelector("#shopItemTitle").value.trim(),description:document.querySelector("#shopItemDescription").value.trim(),emoji:document.querySelector("#shopItemEmoji").value.trim()||"🎁",price_coins:Number(document.querySelector("#shopItemPrice").value),image_url:document.querySelector("#shopItemImage").value.trim()||null,is_active:document.querySelector("#shopItemActive").checked},errorBox=document.querySelector("#shopItemError");const button=event.currentTarget.querySelector('[type="submit"]');errorBox.textContent="";button.disabled=true;try{if(id)await api(`shop_items?id=eq.${id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify(payload)});else await api("shop_items",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify(payload)});document.querySelector("#shopItemDialog").close();toast(id?"Товар обновлён":"Товар добавлен");shop();}catch(error){let message=error.message;try{const parsed=JSON.parse(message);message=parsed.message||parsed.hint||message}catch(_){}errorBox.textContent=`Не удалось сохранить: ${message}`;}finally{button.disabled=false}}
