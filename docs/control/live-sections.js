(function () {
  ...
  async function balances(){
    ...
    try{
      ...
      const balances=users.map(user=>({...user,employee:people.get(String(user.telegram_id))})).filter(row=>row.employee),orphans=users.filter(user=>!people.has(String(user.telegram_id)));
      root.innerHTML=`<div class="stats mini-stats"><div class="stat"><small>Активных счетов</small><strong>${balances.length}</strong></div><div class="stat"><small>Всего спасибок</small><strong>${balances.reduce((sum,row)=>sum+Number(row.balance||0),0)}</strong></div><div class="stat"><small>Всего монет</small><strong>${balances.reduce((sum,row)=>sum+Number(row.coins||0),0)}</strong></div></div>${orphans.length?`<div class="section-heading cleanup-users"><div><h3>Аккаунты без сотрудников</h3><p>${orphans.length} записей не связаны ни с одной карточкой сотрудника. Очистка сохранит аккаунты с историей.</p></div><button class="secondary danger-action" id="cleanupUsers">Очистить</button></div>`:''}<div class="employee-tools"><input id="balanceSearch" placeholder="Найти сотрудника"><span id="balanceCount">${balances.length} сотрудников</span></div><div id="balanceTable"></div><h3>Последние операции</h3><div id="balanceHistory">${balanceHistory(transactions,people)}</div>`;
      ...
      document.querySelector("#balanceSearch").oninput=event=>draw(event.target.value);draw("");const cleanup=document.querySelector("#cleanupUsers");if(cleanup)cleanup.onclick=()=>cleanupUsers(orphans.length);
    }catch(error){fail(root,error)}
  }

  async function cleanupUsers(count){if(!confirm(`Проверить и удалить ${count} аккаунтов без карточки сотрудника? Аккаунты с покупками, начислениями, оценками или правами менеджера останутся.`))return;const button=document.querySelector("#cleanupUsers");button.disabled=true;try{const result=await api("rpc/admin_cleanup_orphan_users",{method:"POST",body:JSON.stringify({p_actor_id:actorId()})});toast(`Удалено: ${Number(result.deleted||0)}. Сохранено с историей: ${Number(result.preserved||0)}.`);balances();}catch(error){toast(error.message)}finally{button.disabled=false}}
  ...
  function editShopItem(item){
    let dialog=document.querySelector("#shopItemDialog");if(!dialog){document.body.insertAdjacentHTML("beforeend",'<dialog class="editor-dialog" id="shopItemDialog"><form id="shopItemForm"><div class="compose-head"><div><span class="eyebrow">МАГАЗИН</span><h2>Карточка награды</h2></div><button type="button" id="shopItemClose">×</button></div><input type="hidden" id="shopItemId"><label>Название<input id="shopItemTitle" required maxlength="100"></label><label>Описание<input id="shopItemDescription" maxlength="300"></label><div class="form-pair"><label>Эмодзи<input id="shopItemEmoji" maxlength="8"></label><label>Цена в монетах<input id="shopItemPrice" type="number" min="1" required></label></div><label>Ссылка на изображение<input id="shopItemImage" type="url"></label><label class="switch-row"><span><strong>Показывать в магазине</strong><small>Скрытый товар нельзя купить</small></span><input id="shopItemActive" type="checkbox"></label><div class="dialog-error" id="shopItemError" role="alert"></div><div class="dialog-actions"><button class="secondary danger-action" id="shopItemDelete" type="button">Удалить товар</button><button class="primary" type="submit">Сохранить товар</button></div></form></dialog>');dialog=document.querySelector("#shopItemDialog");document.querySelector("#shopItemClose").onclick=()=>dialog.close();document.querySelector("#shopItemForm").onsubmit=saveShopItem;document.querySelector("#shopItemDelete").onclick=deleteShopItem;}
    document.querySelector("#shopItemId").value=item.id||0;document.querySelector("#shopItemTitle").value=item.title||"";document.querySelector("#shopItemDescription").value=item.description||"";document.querySelector("#shopItemEmoji").value=item.emoji||"🎁";document.querySelector("#shopItemPrice").value=item.price_coins||"";document.querySelector("#shopItemImage").value=item.image_url||"";document.querySelector("#shopItemActive").checked=item.is_active!==false;document.querySelector("#shopItemError").textContent="";document.querySelector("#shopItemDelete").style.display=item.id?"inline-flex":"none";dialog.showModal();
  }
  ...
  async function deleteShopItem(){const id=Number(document.querySelector("#shopItemId").value),title=document.querySelector("#shopItemTitle").value.trim(),button=document.querySelector("#shopItemDelete"),errorBox=document.querySelector("#shopItemError");if(!id||!confirm(`Удалить товар «${title}» из магазина? История покупок сохранится.`))return;button.disabled=true;errorBox.textContent="";try{await api("rpc/admin_save_shop_item",{method:"POST",body:JSON.stringify({p_actor_id:actorId(),p_item_id:id,p_title:title,p_description:document.querySelector("#shopItemDescription").value.trim(),p_price_coins:Number(document.querySelector("#shopItemPrice").value),p_image_url:document.querySelector("#shopItemImage").value.trim()||null,p_emoji:document.querySelector("#shopItemEmoji").value.trim()||"🎁",p_is_active:false})});document.querySelector("#shopItemDialog").close();toast("Товар удалён из магазина");shop();}catch(error){let message=error.message;try{message=JSON.parse(message).message||message}catch(_){}errorBox.textContent=`Не удалось удалить: ${message}`;}finally{button.disabled=false}}
  ...
  async function notifications() {
    ...
    try{
      const [chats,settings,employees,custom]=await Promise.all([api("chats?type=neq.private&select=*&order=title.asc"),api("bot_settings?select=key,value"),api("employee_profiles?activation_status=eq.active&telegram_id=not.is.null&select=telegram_id,full_name,position&order=full_name.asc"),api("admin_notifications?select=*&order=created_at.desc").catch(()=>null)]);
      ...
      root.innerHTML=`<div class="page-head"><div><h1>Уведомления</h1><p>Разовые и постоянные сообщения в Telegram</p></div><button class="primary compact" id="newNotification">＋ Новое уведомление</button></div><div class="notification-guide"><strong>Как это работает</strong><span>Создай сообщение, выбери чат и сотрудников. Для постоянного уведомления укажи интервал повтора.</span></div>${custom===null?'<div class="role-banner restaurant"><strong>Нужна миграция</strong><span>Примени SQL для произвольных уведомлений — после этого создание станет доступно.</span></div>':`<div class="panel"><h3>Созданные уведомления</h3><div id="customNotificationList">${custom.length?custom.map(row=>`<article class="task-row"><button class="task-check ${row.is_active?'checked':''}" data-notification-toggle="${Number(row.id)}" title="${row.is_active?'Выключить':'Включить'}">${row.is_active?'✓':''}</button><div class="task-main"><div class="task-title">${esc(row.title)}</div><div class="task-meta"><span>${esc(chats.find(chat=>String(chat.chat_id)===String(row.chat_id))?.title||row.chat_id)}</span><span>${row.repeat_hours?`Каждые ${Number(row.repeat_hours)} ч.`:'Один раз'}</span><span>${row.is_active?'Активно':'Выключено'}</span></div><small>${esc(row.message)}</small></div><div class="task-row-actions"><button class="dots" data-notification-edit="${Number(row.id)}" title="Изменить">✎</button><button class="dots" data-notification-delete="${Number(row.id)}" title="Удалить">×</button></div></article>`).join(''):'<div class="empty-live">Уведомлений пока нет</div>'}</div></div>`}<h3>Автоматические сценарии</h3><div class="section-summary">Бот видит <strong>${chats.length}</strong> групп и каналов</div><div class="notification-grid">${channels.map(item=>`<article class="notification-card" data-channel="${item.id}" data-enabled-key="${item.enabled}" data-chat-key="${item.chat}" data-thread-key="${item.thread}"><div class="notification-title"><div><strong>${item.title}</strong><small>${item.description}</small></div><span class="status ${values.get(item.enabled)==='0'?'red':'green'}">${values.get(item.enabled)==='0'?'Выключено':'Работает'}</span></div><label class="switch-row"><span><strong>Постоянные уведомления</strong><small>Бот будет работать автоматически</small></span><input data-enabled type="checkbox" ${values.get(item.enabled)!=='0'?'checked':''}></label><label>Куда отправлять<select data-chat><option value="">Выберите чат</option>${chats.map(chat=>`<option value="${chat.chat_id}" ${String(values.get(item.chat))===String(chat.chat_id)?'selected':''}>${esc(chat.title||`Чат ${chat.chat_id}`)}</option>`).join('')}</select></label><label>Тема Telegram <small>(необязательно)</small><input data-thread type="number" min="0" placeholder="0 — без темы" value="${esc(values.get(item.thread)||0)}"></label>${item.interval?`<label>Повторять<select data-interval><option value="1" ${values.get('schedule_notify_interval_hours')==='1'?'selected':''}>Каждый час</option><option value="2" ${values.get('schedule_notify_interval_hours')==='2'?'selected':''}>Каждые 2 часа</option><option value="4" ${!values.get('schedule_notify_interval_hours')||values.get('schedule_notify_interval_hours')==='4'?'selected':''}>Каждые 4 часа</option><option value="6" ${values.get('schedule_notify_interval_hours')==='6'?'selected':''}>Каждые 6 часов</option><option value="12" ${values.get('schedule_notify_interval_hours')==='12'?'selected':''}>Каждые 12 часов</option></select></label>`:''}<button class="primary compact save-notification">Сохранить сценарий</button><small class="notification-saved"></small></article>`).join('')}</div>`;
      document.querySelector("#newNotification").onclick=()=>editNotification({},chats,employees);
      if(custom){root.querySelectorAll("[data-notification-edit]").forEach(button=>button.onclick=()=>editNotification(custom.find(row=>Number(row.id)===Number(button.dataset.notificationEdit)),chats,employees));root.querySelectorAll("[data-notification-toggle]").forEach(button=>button.onclick=async()=>{const row=custom.find(item=>Number(item.id)===Number(button.dataset.notificationToggle));await api(`admin_notifications?id=eq.${row.id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({is_active:!row.is_active,updated_at:new Date().toISOString()})});toast(row.is_active?"Уведомление выключено":"Уведомление включено");notifications()});root.querySelectorAll("[data-notification-delete]").forEach(button=>button.onclick=async()=>{if(!confirm("Удалить это уведомление?"))return;await api(`admin_notifications?id=eq.${Number(button.dataset.notificationDelete)}`,{method:"DELETE",headers:{Prefer:"return=minimal"}});toast("Уведомление удалено");notifications()})}
  ...
  function editNotification(row,chats,employees){let dialog=document.querySelector("#notificationDialog");if(!dialog){document.body.insertAdjacentHTML("beforeend",'<dialog class="editor-dialog" id="notificationDialog"><form id="notificationForm"><div class="compose-head"><div><span class="eyebrow">УВЕДОМЛЕНИЕ</span><h2>Новое уведомление</h2></div><button type="button" id="notificationClose">×</button></div><input type="hidden" id="notificationId"><label>Название<input id="notificationTitle" maxlength="100" required></label><label>Текст<textarea id="notificationMessage" maxlength="2000" required></textarea></label><label>Telegram-чат<select id="notificationChat" required></select></label><label>ID темы <small>(0 — общий чат)</small><input id="notificationThread" type="number" min="0" value="0"></label><label>Кого отметить<select id="notificationAudience"><option value="all">Всех активных сотрудников</option><option value="selected">Выбранных сотрудников</option></select></label><label id="notificationRecipientsLabel">Сотрудники<select id="notificationRecipients" multiple size="7"></select><small class="field-help">Выбирай несколько строк с Ctrl или Shift.</small></label><label>Отправка<select id="notificationRepeat"><option value="">Один раз</option><option value="1">Каждый час</option><option value="2">Каждые 2 часа</option><option value="4">Каждые 4 часа</option><option value="6">Каждые 6 часов</option><option value="12">Каждые 12 часов</option><option value="24">Раз в сутки</option><option value="168">Раз в неделю</option></select></label><label>Первая отправка<input id="notificationStart" type="datetime-local" required></label><p class="login-error" id="notificationError"></p><button class="primary" type="submit">Сохранить уведомление</button></form></dialog>');dialog=document.querySelector("#notificationDialog");document.querySelector("#notificationClose").onclick=()=>dialog.close();document.querySelector("#notificationForm").onsubmit=saveNotification;document.querySelector("#notificationAudience").onchange=()=>document.querySelector("#notificationRecipientsLabel").classList.toggle("hidden",document.querySelector("#notificationAudience").value==='all');}dialog.dataset.employees=JSON.stringify(employees.map(employee=>Number(employee.telegram_id)));document.querySelector("#notificationId").value=row.id||0;document.querySelector("#notificationTitle").value=row.title||"";document.querySelector("#notificationMessage").value=row.message||"";document.querySelector("#notificationChat").innerHTML='<option value="">Выберите чат</option>'+chats.map(chat=>`<option value="${Number(chat.chat_id)}">${esc(chat.title||chat.chat_id)}</option>`).join('');document.querySelector("#notificationChat").value=String(row.chat_id||"");document.querySelector("#notificationThread").value=row.thread_id||0;document.querySelector("#notificationAudience").value=row.audience||"all";document.querySelector("#notificationRecipients").innerHTML=employees.map(employee=>`<option value="${Number(employee.telegram_id)}" ${(row.recipient_ids||[]).map(String).includes(String(employee.telegram_id))?'selected':''}>${esc(employee.full_name)}${employee.position?` · ${esc(employee.position)}`:''}</option>`).join('');document.querySelector("#notificationRepeat").value=row.repeat_hours||"";const start=row.next_send_at?new Date(row.next_send_at):new Date(Date.now()+60000);start.setMinutes(start.getMinutes()-start.getTimezoneOffset());document.querySelector("#notificationStart").value=start.toISOString().slice(0,16);document.querySelector("#notificationError").textContent="";document.querySelector("#notificationAudience").dispatchEvent(new Event("change"));dialog.showModal()}

  async function saveNotification(event){event.preventDefault();const form=event.currentTarget,button=form.querySelector('[type="submit"]'),error=document.querySelector("#notificationError"),id=Number(document.querySelector("#notificationId").value),audience=document.querySelector("#notificationAudience").value,all=JSON.parse(document.querySelector("#notificationDialog").dataset.employees||"[]"),selected=[...document.querySelector("#notificationRecipients").selectedOptions].map(option=>Number(option.value));const recipients=audience==='all'?all:selected;if(!recipients.length){error.textContent="Выбери хотя бы одного сотрудника.";return}button.disabled=true;error.textContent="";try{const payload={title:document.querySelector("#notificationTitle").value.trim(),message:document.querySelector("#notificationMessage").value.trim(),chat_id:Number(document.querySelector("#notificationChat").value),thread_id:Number(document.querySelector("#notificationThread").value)||null,audience,recipient_ids:recipients,repeat_hours:Number(document.querySelector("#notificationRepeat").value)||null,next_send_at:new Date(document.querySelector("#notificationStart").value).toISOString(),is_active:true,created_by:actorId(),updated_at:new Date().toISOString()};if(id){delete payload.created_by;await api(`admin_notifications?id=eq.${id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify(payload)})}else await api("admin_notifications",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify(payload)});document.querySelector("#notificationDialog").close();toast(id?"Уведомление обновлено":"Уведомление создано");notifications()}catch(cause){error.textContent=cause.message.includes("admin_notifications")?"Сначала примени SQL-миграцию уведомлений.":cause.message}finally{button.disabled=false}}
