(function () {
  ...
  function editShopItem(item){
    let dialog=document.querySelector("#shopItemDialog");if(!dialog){document.body.insertAdjacentHTML("beforeend",'<dialog class="editor-dialog" id="shopItemDialog"><form id="shopItemForm"><div class="compose-head"><div><span class="eyebrow">МАГАЗИН</span><h2>Карточка награды</h2></div><button type="button" id="shopItemClose">×</button></div><input type="hidden" id="shopItemId"><label>Название<input id="shopItemTitle" required maxlength="100"></label><label>Описание<input id="shopItemDescription" maxlength="300"></label><div class="form-pair"><label>Эмодзи<input id="shopItemEmoji" maxlength="8"></label><label>Цена в монетах<input id="shopItemPrice" type="number" min="1" required></label></div><label>Ссылка на изображение<input id="shopItemImage" type="url"></label><label class="switch-row"><span><strong>Показывать в магазине</strong><small>Скрытый товар нельзя купить</small></span><input id="shopItemActive" type="checkbox"></label><button class="primary" type="submit">Сохранить товар</button></form></dialog>');dialog=document.querySelector("#shopItemDialog");document.querySelector("#shopItemClose").onclick=()=>dialog.close();document.querySelector("#shopItemForm").onsubmit=saveShopItem;}
    document.querySelector("#shopItemId").value=item.id||0;document.querySelector("#shopItemTitle").value=item.title||"";document.querySelector("#shopItemDescription").value=item.description||"";document.querySelector("#shopItemEmoji").value=item.emoji||"🎁";document.querySelector("#shopItemPrice").value=item.price_coins||"";document.querySelector("#shopItemImage").value=item.image_url||"";document.querySelector("#shopItemActive").checked=item.is_active!==false;dialog.showModal();
  }

  async function saveShopItem(event){event.preventDefault();const id=Number(document.querySelector("#shopItemId").value),payload={title:document.querySelector("#shopItemTitle").value.trim(),description:document.querySelector("#shopItemDescription").value.trim(),emoji:document.querySelector("#shopItemEmoji").value.trim()||"🎁",price_coins:Number(document.querySelector("#shopItemPrice").value),image_url:document.querySelector("#shopItemImage").value.trim()||null,is_active:document.querySelector("#shopItemActive").checked};const button=event.currentTarget.querySelector('[type="submit"]');button.disabled=true;try{if(id)await api(`shop_items?id=eq.${id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify(payload)});else await api("shop_items",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify(payload)});document.querySelector("#shopItemDialog").close();toast(id?"Товар обновлён":"Товар добавлен");shop();}catch(error){toast(error.message)}finally{button.disabled=false}}
